...
test('409 assignment conflict informs the user and refreshes dashboard state', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => { const path = new URL(route.request().url()).pathname; const json = (body: unknown) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(body) });
    ...
    if (path === '/api/dashboard/agents') return json({ agents: [{ id: 'agent-id', displayName: 'Agent Demo', status: 'Available', lastAvailableAtUtc: null, currentCallReference: null, callQueueNames: ['Support'] }] }); if (path === '/api/dashboard/calls') { callReads++; return json({ waitingCalls: [waitingCall], activeCalls: [] }); }
    if (path === '/api/calls/call-id/assign/agent-id') return route.fulfill({ status: 409, contentType: 'application/json', body: '{}' }); return route.fulfill({ status: 404, body: '{}' }); });
