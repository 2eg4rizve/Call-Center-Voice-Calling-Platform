...
test('Supervisor creates and assigns a call, then reviews refreshed history and details', async ({ page }) => {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/dashboard/agents') return json({ agents: [{ id: 'agent-id', displayName: 'Agent Demo', status: phase === 'assigned' ? 'Busy' : 'Available', lastAvailableAtUtc: null, currentCallReference: phase === 'assigned' ? call.callReferenceNumber : null, callQueueNames: ['Support'] }] });
    ...
    if (path === '/api/calls/call-id/assign/agent-id') { phase = 'assigned'; return json({ ...call, status: 'Assigned', assignedAgentId: 'agent-id', assignedAgentName: 'Agent Demo', assignedAtUtc: '2026-07-23T10:00:05Z' }); }
  ...
  const [assignRequest] = await Promise.all([page.waitForRequest('**/api/calls/call-id/assign/agent-id'), page.getByRole('button', { name: 'Assign', exact: true }).click()]);
