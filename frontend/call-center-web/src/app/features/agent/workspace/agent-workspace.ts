...
export class AgentWorkspace {
  ...
  protected loadProfile(): void { this.agents.current().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (p) => { this.profile.set(p); this.error.set(null); }, error: () => this.error.set('Unable to load the agent profile.') }); }
  protected changeStatus(status: AgentStatus): void { if (this.pending() || this.currentCall()) return; this.pending.set(true); this.agents.updateStatus({ status }).pipe(finalize(() => this.pending.set(false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => this.loadProfile(), error: () => this.error.set('Unable to update the status.') }); }
  protected accept(): void { const call = this.currentCall(); if (!call || call.status !== 'Assigned' || this.pending()) return; this.pending.set(true); this.calls.accept(call.id).pipe(finalize(() => this.pending.set(false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: (updated) => { this.currentCall.set(updated); this.updateElapsed(); this.loadProfile(); }, error: (e: ApiError) => { this.error.set(e.status === 409 ? 'Call state changed. Loading the latest current call.' : 'Unable to accept the call.'); if (e.status === 409) this.currentRefresh.next(); } }); }
  protected complete(): void { const call = this.currentCall(); if (!call || this.outcome.invalid || this.notes.invalid || this.pending()) { this.outcome.markAsTouched(); return; } this.pending.set(true); this.calls.complete(call.id, { outcome: this.outcome.value!, notes: this.notes.value.trim() || null }).pipe(finalize(() => this.pending.set(false)), takeUntilDestroyed(this.destroyRef)).subscribe({ next: () => { this.currentCall.set(null); this.outcome.reset(); this.notes.reset(''); this.loadProfile(); this.currentRefresh.next(); }, error: () => this.error.set('Unable to complete the call. Your notes have been preserved; please try again.') }); }
