...
export class AgentAdministration {
  ...
  protected create():void{this.formError.set(null);if(this.createForm.invalid||this.pending()){this.createForm.markAllAsTouched();this.formError.set('Required field-gulo shothik vabe complete korun.');return}this.pending.set(true);this.api.create(this.createForm.getRawValue()).pipe(finalize(()=>{this.pending.set(false);this.createForm.controls.password.reset('')}),takeUntilDestroyed(this.destroyRef)).subscribe({next:()=>{this.createForm.reset({fullName:'',displayName:'',email:'',password:''});this.notify.show('Agent created successfully.');this.load()},error:(e:ApiError)=>{if(e.validationErrors)applyServerValidationErrors(this.createForm,e.validationErrors);this.formError.set(e.message||'Agent create kora jayni. Input-gulo check korun.')}});}
  ...
  protected update():void{const agent=this.selected();this.formError.set(null);if(!agent||this.editForm.invalid||this.pending()){this.editForm.markAllAsTouched();this.formError.set('Valid display name din.');return}this.pending.set(true);this.api.update(agent.id,this.editForm.getRawValue()).pipe(finalize(()=>this.pending.set(false)),takeUntilDestroyed(this.destroyRef)).subscribe({next:()=>{this.notify.show('Agent name updated.');this.selected.set(null);this.load()},error:(e:ApiError)=>{if(e.validationErrors)applyServerValidationErrors(this.editForm,e.validationErrors);this.formError.set(e.message||'Agent update kora jayni.')}});}
  protected assign():void{this.formError.set(null);if(this.assignmentForm.invalid||this.pending()){this.assignmentForm.markAllAsTouched();this.formError.set('Assignment-er jonno ekta queue select korun.');return}this.pending.set(true);this.api.assignToQueue(this.assignmentForm.getRawValue()).pipe(finalize(()=>this.pending.set(false)),takeUntilDestroyed(this.destroyRef)).subscribe({next:()=>this.notify.show('Queue assignment saved. Existing assignments remain unchanged.'),error:(e:ApiError)=>this.formError.set(e.message||'Queue assignment failed.')});}
}
